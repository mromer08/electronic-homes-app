...
public interface VentaRepository extends JpaRepository<Venta, Integer>{
    ...
    public Optional<Venta> findByCliente(@Param("id_cliente") Integer id_cliente, Pageable pageable);
    ...
    public Optional<Venta> findByEmpleado(@Param("id_empleado") Integer id_empleado);
    ...
    public Optional<Venta> findById(@Param("id_empleado") Integer id);

    @Query("SELECT v.*, SUM(dv.cantidad * p.precio) * (1 - v.descuento) as total FROM ControlVentas.Venta v INNER JOIN ControlVentas.DetalleVenta dv ON v.id = dv.id_venta INNER JOIN ControlInventario.Producto p ON dv.id_producto = p.id GROUP BY v.id")
